...
import { FilePlus, Tag, MoreHorizontal, Send, XCircle, Plus } from "lucide-react";
import { toast } from "sonner";
import {
  getDebitNotesQuery,
  issueDebitNote,
  cancelDebitNote,
} from "@/lib/actions/debit-notes";
...
import { NewDebitNoteDrawer } from "@/components/forms/new-debit-note-drawer";

const STATUS_OPTIONS = Object.keys(CREDIT_NOTE_STATUS_LABELS).map((s) => ({
...
export default function NotasDebitoPage() {
  ...
  const [items, setItems] = React.useState<DebitNote[]>([]);
  ...
  React.useEffect(() => {
    if (searchParams.get("new") === "1") {
      ...
      router.replace("/comprobantes/notas-debito", { scroll: false });
    }
  }, [searchParams, router]);

  const columns = React.useMemo<ColumnDef<DebitNote>[]>(
    () => [
      {
        id: "q",
        accessorFn: () => "",
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", statusColors[row.original.status] ?? "bg-muted")}>
            {CREDIT_NOTE_STATUS_LABELS[row.original.status] ?? row.original.status}
          </span>
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const canIssue = can("debit_notes:issue") && (n.status === "draft" || n.status === "pending_issue");
          const canCancel = can("debit_notes:cancel") && n.status !== "voided";
                ...
                <DropdownMenuContent align="end" className="w-40">
                  {canIssue && (
                    <DropdownMenuItem onSelect={async () => {
                      try { await issueDebitNote(n.id); toast.success("Nota emitida"); fetchPageRef.current?.(); }
                  ...
                  {canCancel && (
                    <>
                      ...
                      <DropdownMenuItem variant="destructive" onSelect={async () => {
                        if (!confirm("¿Anular nota de débito?")) return;
                        try { await cancelDebitNote(n.id); toast.success("Nota anulada"); fetchPageRef.current?.(); }
                        catch (e) { toast.error(e instanceof Error ? e.message : "Error"); }
                      }}>
                        <XCircle /> Anular
                      </DropdownMenuItem>
  ...
  const fetchPage = React.useCallback(() => {
    ...
    return getDebitNotesQuery(filterParams)
      ...
      <div className="mx-auto w-full space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-semibold flex items-center gap-2">
              <FilePlus className="size-6 text-p3" />Notas de débito
              <PageHelpTooltip content={SCREEN_HELP["comprobantes/notas-debito"]} />
            </h1>
            <p className="text-sm text-muted-foreground mt-0.5">{totalRows} notas de débito</p>
          </div>
        ...
        <ERPDataTable table={table} loading={loading} skeletonColumnCount={7}>
          <ExportButton
            ...
            {...exportRows}
            filename="notas-debito"
          />
        </ERPDataTable>
      </div>
      <NewDebitNoteDrawer open={newDrawerOpen} onOpenChange={setNewDrawerOpen} onCreated={() => fetchPageRef.current?.()} />
    </div>
  );
}
