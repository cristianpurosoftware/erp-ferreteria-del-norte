...
import { createDebitNote } from "@/lib/actions/debit-notes";
...
export function DebitNoteForm({ open, onOpenChange, onCreated }: Props) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success("Nota de débito creada como borrador");
      onCreated();
      onOpenChange(false);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Error al crear nota de débito");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div>
            <Label required>Importe a debitar</Label>
            <Input
              type="number"
              min="0.01"
              step="0.01"
            ...
            {selected && (
              <p className="text-xs text-muted-foreground mt-1">
                Referencia: {formatMoney(Number(selected.total))}
              </p>
            )}
          </div>
